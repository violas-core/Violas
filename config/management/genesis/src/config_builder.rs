...
use crate::{
    swarm_config::BuildSwarm, validator_builder::ValidatorBuilder,
...
impl BuildSwarm for FullnodeBuilder {
    fn build_swarm<R>(
        &self,
        _rng: R,
    ) -> anyhow::Result<(Vec<NodeConfig>, Ed25519PrivateKey, ChainId)>
    where
        R: ::rand::RngCore + ::rand::CryptoRng,
    {
        ...
        Ok((configs, diem_root_key_path, chain_id))
    }
}
...
pub fn test_config() -> (NodeConfig, Ed25519PrivateKey) {
    ...
    let (mut configs, key, _chain_id) = builder.build_swarm(rand::rngs::OsRng).unwrap();
